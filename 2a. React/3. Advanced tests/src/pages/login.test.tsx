...
describe("Login and Authentication Flow", () => {
  ...
  it("should show validation error for an invalid email", async () => {
    ...
    expect(
      await screen.findByText(/adres email musi kończyć się na @student.pwr.edu.pl/i)
    ).toBeInTheDocument();
  });
  ...
  it("should successfully log in and redirect to /plans on correct credentials", async () => {
    ...
    expect(await screen.findByText(/przeglądaj i porównuj różne wersje planu/i)).toBeInTheDocument();
  });
  ...
  it("should show an error for incorrect OTP", async () => {
    ...
    expect(await screen.findByText(/nieprawidłowy kod otp lub email/i)).toBeInTheDocument();
  });
  ...
  it("should redirect from a protected route to login page if not authenticated", async () => {
    ...
    expect(screen.queryByText(/przeglądaj i porównuj różne wersje planu/i)).not.toBeInTheDocument();
  });
  ...
  it("should allow a logged-in user to log out", async () => {
    ...
    const logoutButton = await screen.findByRole('button', { name: /wyloguj/i });

    await user.click(logoutButton);

    expect(await screen.findByText(/zaloguj się do planera/i)).toBeInTheDocument();
  });
});