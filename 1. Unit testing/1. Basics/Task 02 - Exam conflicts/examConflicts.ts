...
export function parseExamSchedule(
  json: string,
): Exam[] {
  let arr: any;
  try {
  ...
  return arr.map(
    (item, idx) => {
      ...
      if (
        typeof item.registrationDeadline !==
        "string"
      )
        errs.push(
      ...
      if (
        ...
        throw new Error(
          `Invalid date format at index ${idx}: ${item.date}`,
        );
      }
      if (
        isNaN(
      ...
      if (
        ...
        throw new Error(
          `Registration deadline after exam date at index ${idx}`,
        );
      }
...
export function canRegister(
  ...
  return (
    now.getTime() <=
    exam.registrationDeadline.getTime()
  );
}
...
export function computeRegistrationFee(
  ...
  if (
    now.getTime() <=
    exam.earlyBirdDeadline.getTime()
  ) {
    return +(
      exam.fee * 0.8
    ).toFixed(2);
