...
export class SupervisorManager {
  ...
  private async getAllSupervisors(): Promise<
    Supervisor[]
  > {
    return db.sql(
  ...
  async addSupervisor(
    ...
    if (
      ...
      throw new Error(
        `Supervisor with id ${sp.id} already exists`,
      );
    }
  ...
  async removeSupervisor(
    ...
    if (
      ...
      throw new Error(
        `Supervisor with id ${id} not found`,
      );
    }
  }
  ...
  async updateSupervisor(
    ...
    await db.sql(
      `UPDATE supervisors SET ${setClause} WHERE id = $1`,
      values,
    );
  }
      ...
      supervisors
        ...
        .sort(
          (
            ...
            return a.sp.name.localeCompare(
              b.sp
                .name,
            );
          },
